...
def features(location):
    ...
    # return the standardised(-pi< < pi) value of an angle(radian) 
    ...
    for files in os.listdir(location):
        for name in os.listdir(location+"/"+files):
            #time.sleep(10)
            name = location+"/"+files+"/"+name
            file = open(name, 'r')
            lines = file.readlines()
            startfrom = 7
            j = 0
            for line in lines:
                if j > 6:   # ignore the first siz lines of the data file
                    values = line.split(',')    # comma separated values
                    if values[0] == 'MM' and len(values) == 4:  # checking useCase and validating length of array  
                        if int(values[3]) != 0:
